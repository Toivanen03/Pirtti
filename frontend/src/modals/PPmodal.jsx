...
const PPmodal = ({ showModal, setShowModal, mobile, portrait, data, loading, isFB }) => {
    ...
    useEffect(() => {
        if (!loading && data) {
            setFileName(data.privacyPolicyDocument?.filename)
            setPdfData(data.privacyPolicyDocument?.pdf)
        }
    ...
    return (
        <Modal id="pp-modal" show={showModal} size="xl" className='mb-5' centered={mobile}>
            <Modal.Header>
                <strong>Pirtti ry:n tietosuojaseloste</strong>
            </Modal.Header>
            ...
            <Modal.Footer className="d-flex justify-content-center gap-5">
                {!mobile ? (
                    <Button variant='success'
                        onClick={() => {
                            ...
                            link.href = pdf
                            link.download = fileName || 'tietosuojaseloste.pdf'
                ...
                {!mobile &&
                    <Button variant='info'
                        onClick={() => {
                            const iframe = document.querySelector('#pp-modal iframe')
                            if (iframe) {
